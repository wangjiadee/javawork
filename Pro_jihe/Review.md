...
```
1.set里面的元素不能重复，是用equals来区分的， equals()和==方法判断引用值是否指向同一对象
```
...
说出ArrayList,Vector, LinkedList的存储性能和特性？ArrayList和Vector的区别

```
ArrayList和Vector都使用数组的方式存储数据，此数组元素（16）大于实际存储的数据以便增加和插入元素，它们都允许直接按序号索引元素，但是插入元素要涉及数组元素移动等内存操作，所以索引数据快 而插入数据慢.Vector老的方式 线程安全  LinkedList是具有双向链表结构，插入速度快。
```

|        | ArrayList                   | Vector                      |
| ------ | --------------------------- | --------------------------- |
| 不同点 | 线程不安全 非同步           | 线程安全 同步               |
| 不同点 | 数据扩容 原来的1倍数 （*2） | 数据扩容 原来的一半（*1.5） |
...
```
HashMap 和Hashtable 都是实现MAP接口，两个算法差不多，性能一样
```

|         | HashMap                          | Hashtable           |
| ------- | -------------------------------- | ------------------- |
| 不同点1 | 线程不安全 相对效率高            | 线程安全 相对效率低 |
| 不同点2 | 允许元素为空 k=null              | 不允许元素为空      |
| 不同点3 | containsvalue和containsKey的方法 | contains方法        |
| 不同点4 | 继承Map                          | 继承Dirctionary     |
